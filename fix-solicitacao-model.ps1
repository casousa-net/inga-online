...
$apiDir = "c:\Users\Orlando Nzagi\Desktop\inga-online\app\api\solicitacoes"
...
ForEach-Object {
    $content = Get-Content $_.FullName
    $newContent = $content -replace "itens: ", "solicitacaoitem: "
    if ($content -ne $newContent) {
        Set-Content -Path $_.FullName -Value $newContent
...
ForEach-Object {
    $content = Get-Content $_.FullName
    $newContent = $content -replace "documentos: ", "documentosolicitacao: "
    if ($content -ne $newContent) {
        Set-Content -Path $_.FullName -Value $newContent
